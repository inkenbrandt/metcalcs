...
def test_calculate_radiation_typical():
    ...
    assert all(k in result for k in ['extraterrestrial', 'net_shortwave', 'net_longwave', 'net_radiation'])
...
def test_solar_hour_angle():
    time = datetime.datetime(2023, 6, 21, 12, 0)
    angle = radiation.solar_hour_angle(-112.0, time)
    assert abs(angle) < 0.1  # Near solar noon
...
def test_incoming_solar_radiation_daytime():
    rad = radiation.incoming_solar_radiation(40.0, -112.0, datetime.datetime(2023, 6, 21, 12, 0))
    assert rad > 0
...
def test_estimate_clear_sky_radiation_day():
    result = radiation.estimate_clear_sky_radiation(40, -112, datetime.datetime(2023, 6, 21, 12, 0))
    assert result > 0
